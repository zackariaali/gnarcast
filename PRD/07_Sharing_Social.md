...
> Sub-spec 07 | Status: **LOCKED (with open questions)** | Last Updated: 2026-05-16
...
### Relationship to Scout Templates

Popular Setups occupy the same UX slot as Scout Templates (see `02_User_Preferences.md`) but are data-derived rather than hand-curated. The model:

- A mountain with fewer than the minimum active Scouts shows **Templates only** (FIRST TRACKS, POW HUNT, etc.).
- A mountain at or above the minimum threshold shows **Popular Setups** at the top of the creation flow, with **Templates available below** as a "Start from a generic archetype" option.
- Popular Setups take precedence because they reflect how riders actually configure *this specific* mountain — more accurate signal than a generic template once data exists.

This means every mountain has *something* to pick from at every stage of the product's life — there is never a blank starting point.
...
>
> [POW HUNT — Crystal style]   [BLUEBIRD — Crystal style]   [PARK DAY — Crystal style]
>
> ▾ Start from a generic template instead — FIRST TRACKS · STORM CHASER · POW HUNT · BLUEBIRD · PARK DAY · ROAD TRIP
>
> Start from scratch →
...
- Where a derived archetype clearly aligns with a canonical template (POW HUNT, BLUEBIRD, etc.), the LLM names it with the canonical name plus a mountain qualifier — e.g., "POW HUNT — Crystal style." This keeps the vocabulary consistent with the rest of the product.
...
Each archetype gets a short name generated by the LLM based on its dominant conditions. Where possible the LLM prefers a canonical template name (see `02_User_Preferences.md`) with a mountain qualifier, since reusing the canonical vocabulary keeps the product's language consistent:

- High new snow weight + active snowfall → **"STORM CHASER — Crystal style"**
- High new snow weight + low crowd weight + post-storm → **"POW HUNT — Crystal style"**
- High grooming weight + bluebird weight + early-day → **"FIRST TRACKS — Crystal style"**
- Park-specific conditions weighted high → **"PARK DAY — Crystal style"**
- Configurations that don't map cleanly to a canonical template → LLM generates a fresh short name (uppercase, 1–2 words, ski-culture resonant)
